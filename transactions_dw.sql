...
-- Berapa banyak penjualan total revenue di Quarter 1 2024

SELECT SUM(revenue) AS total_revenue
FROM data_warehouse.sales_fact as sf
JOIN data_warehouse.time_dimension as td ON sf.date_id = td.date_id
WHERE td.year = 2024 AND td.quarter = '1';

-- Berapa banyak penjualan secara quantity di quarter 4 2023
SELECT SUM(quantity) AS total_quantity
FROM data_warehouse.sales_fact as sf
JOIN data_warehouse.time_dimension as td ON sf.date_id = td.date_id
WHERE td.year = 2023 AND td.quarter = '4';

-- Dari semua data penjualan yang ada carilah di Quarter berapa dan tahun berapa penjualan paling banyak secara revenue
SELECT td.quarter, td.year, SUM(sf.revenue) AS total_revenue
FROM data_warehouse.sales_fact as sf
JOIN data_warehouse.time_dimension as td ON sf.date_id = td.date_id
